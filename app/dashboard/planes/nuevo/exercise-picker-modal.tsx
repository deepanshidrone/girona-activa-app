...
import { X, Search, Plus, Check, ChevronLeft, Pencil, Trash2 } from 'lucide-react'
import { Label } from '@/components/ui/label'
import { Button } from '@/components/ui/button'
import { PlanExercise } from '@/app/actions/plans'
...
interface Props {
  ...
  onUpdate: (index: number, sets: number, reps: number, weight_kg: string, notes: string) => void
  onClose: () => void
}
...
type EditingExisting = { index: number; sets: number; reps: number; weight_kg: string; notes: string }

export function ExercisePickerModal({
  exercises, exerciseCatalog, existingExercises, bodyZones, movementPatterns, muscleGroups, equipment, objectives, dayLabel, onAdd, onRemove, onUpdate, onClose
}: Props) {
  ...
  const [editingExisting, setEditingExisting] = useState<EditingExisting | null>(null)
  ...
  function handleUpdateExisting() {
    ...
    onUpdate(editingExisting.index, editingExisting.sets, editingExisting.reps, editingExisting.weight_kg, editingExisting.notes)
    setEditingExisting(null)
  }

  function startEditExisting(i: number) {
  ...
  const getExerciseName = (id: string) => exerciseCatalog.find(e => e.id === id)?.name ?? id

  const mainContent = editingExisting ? (
    /* Editar ejercicio existente */
    <div className="flex-1 flex items-center justify-center p-6">
      <div className="w-full max-w-sm">
        <button onClick={() => setEditingExisting(null)} className="flex items-center gap-1.5 text-white/40 hover:text-white text-sm mb-4 transition-colors">
          <ChevronLeft className="h-4 w-4" /> Volver
        </button>
        <div className="bg-[#FF914D]/10 border border-[#FF914D]/20 rounded-xl p-4 mb-4">
          <p className="font-semibold text-white">{getExerciseName(existingExercises[editingExisting.index].exercise_id)}</p>
          <p className="text-xs text-white/40 mt-0.5">Editando ejercicio {editingExisting.index + 1}</p>
        </div>
        <div className="grid grid-cols-2 gap-3 mb-3">
          <div className="flex flex-col gap-1.5">
            <Label className="text-xs text-white/60">Series</Label>
            <input type="number" value={editingExisting.sets} min={1} max={20}
              onChange={e => setEditingExisting(v => v ? { ...v, sets: parseInt(e.target.value) || 1 } : v)}
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <input type="number" value={editingExisting.reps} min={1} max={100}
              onChange={e => setEditingExisting(v => v ? { ...v, reps: parseInt(e.target.value) || 1 } : v)}
              className="bg-[#111111] border border-white/10 rounded-lg px-3 py-2 text-sm text-white outline-none focus:border-[#FF914D] transition-colors" />
          </div>
        </div>
        <div className="flex flex-col gap-1.5 mb-3">
          <Label className="text-xs text-white/60">Peso (kg) — opcional</Label>
          <input type="number" value={editingExisting.weight_kg} placeholder="Ej: 60" step={0.5}
            onChange={e => setEditingExisting(v => v ? { ...v, weight_kg: e.target.value } : v)}
        ...
        <div className="flex flex-col gap-1.5 mb-4">
          ...
          <input value={editingExisting.notes} placeholder="Indicaciones..."
            onChange={e => setEditingExisting(v => v ? { ...v, notes: e.target.value } : v)}
            className="bg-[#111111] border border-white/10 rounded-lg px-3 py-2 text-sm text-white placeholder-white/20 outline-none focus:border-[#FF914D] transition-colors" />
        </div>
        <Button onClick={handleUpdateExisting} className="bg-[#FF914D] hover:bg-[#e07a3a] text-white font-bold w-full gap-2">
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
      <div className="w-full max-w-5xl bg-[#1C1C1C] rounded-2xl border border-white/10 shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-white/10 shrink-0">
          <div>
            <h2 className="font-bold text-white text-lg">Ejercicios del día</h2>
        ...
        <div className="flex flex-1 min-h-0">
          {/* Sidebar filtros */}
          <aside className="w-44 shrink-0 border-r border-white/10 p-4 flex flex-col gap-3 overflow-y-auto">
          ...
          {/* Panel derecho — ejercicios del día */}
          <aside className="w-56 shrink-0 border-l border-white/10 flex flex-col">
            <div className="px-4 py-3 border-b border-white/10 shrink-0">
              <p className="text-xs font-semibold text-white/60 uppercase tracking-wider">
                En este día
                {existingExercises.length > 0 && (
                  <span className="ml-1.5 bg-[#FF914D]/20 text-[#FF914D] text-[10px] px-1.5 py-0.5 rounded-full font-bold">
            ...
            {existingExercises.length === 0 ? (
              <div className="flex-1 flex flex-col items-center justify-center p-4 text-center">
                <p className="text-white/20 text-xs">Todavía no hay ejercicios en este día</p>
              </div>
            ) : (
              <div className="flex-1 overflow-y-auto p-3 flex flex-col gap-2">
                {existingExercises.map((ex, i) => (
                  <div key={i} className={`bg-[#111111] rounded-xl border p-3 transition-colors ${
                    editingExisting?.index === i ? 'border-[#FF914D]/50' : 'border-white/10'
                  }`}>
                    <p className="text-xs font-semibold text-white leading-tight mb-1.5">
                      {i + 1}. {getExerciseName(ex.exercise_id)}
                    </p>
                    ...
                    <div className="flex gap-1.5">
                      <button
                        ...
                        className="flex-1 flex items-center justify-center gap-1 py-1 rounded-lg text-[10px] text-white/50 hover:text-[#FF914D] bg-white/5 hover:bg-[#FF914D]/10 border border-white/10 hover:border-[#FF914D]/30 transition-colors"
                      >
                        <Pencil className="h-2.5 w-2.5" /> Editar
                      </button>
                      <button
                        onClick={() => onRemove(i)}
                        className="flex items-center justify-center px-2 py-1 rounded-lg text-[10px] text-white/30 hover:text-red-400 bg-white/5 hover:bg-red-500/10 border border-white/10 hover:border-red-500/30 transition-colors"
                      >
