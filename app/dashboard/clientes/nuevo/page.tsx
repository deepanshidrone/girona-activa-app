...
export default function NuevoClientePage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (authError || !authData?.user) {
      // Si falla auth, intentamos igualmente crear el cliente sin user_id
      // para no bloquear el flujo (el link de auth se puede hacer después)
    }

    // 2. Insertar cliente en la tabla clients
    ...
    // 3. Enviar email de recuperación para que el cliente establezca su contraseña
    if (authData?.user) {
    ...
    <div className="p-6 max-w-2xl mx-auto">
      ...
      <div className="flex items-center gap-3 mb-6">
        <Link
          ...
          className="text-[#666666] hover:text-[#1C1C1C] transition-colors"
        >
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-[#E5E5E5] p-6 flex flex-col gap-5">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <p className="text-xs text-[#666666]">
            Se enviará un email al cliente para que establezca su contraseña de acceso.
          </p>
        </div>
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <Textarea
            ...
            value={form.notes}
            onChange={handleChange}
            placeholder="Observaciones, lesiones previas, objetivos específicos..."
            rows={3}
          />
        </div>
