          ...
          {mySessions.length > 0 && (
            <section>
              <div className="flex items-center gap-2 mb-3">
                <span className="text-xs font-semibold text-[#FF914D] uppercase tracking-wider">Mis sesiones</span>
                <div className="flex-1 h-px bg-[#FF914D]/20" />
                <span className="text-xs text-white/30">{mySessions.length}</span>
              </div>
              <div className="flex flex-col gap-2">
                {mySessions.map((s: any) => <SessionCard key={s.id} session={s} highlight />)}
              </div>
            </section>
          )}

          {/* Sesiones de otros entrenadores */}
          {[...byEmployee.entries()].map(([empId, { name, sessions: empSessions }]) => (
            <section key={empId}>
              <div className="flex items-center gap-2 mb-3">
                <User className="h-3.5 w-3.5 text-white/30" />
                <span className="text-xs font-semibold text-white/50 uppercase tracking-wider">{name}</span>
                <div className="flex-1 h-px bg-white/10" />
                <span className="text-xs text-white/30">{empSessions.length}</span>
              </div>
              <div className="flex flex-col gap-2">
                {empSessions.map((s: any) => <SessionCard key={s.id} session={s} highlight={false} />)}
              </div>
            </section>
          ))}
...
function SessionCard({ session, highlight }: { session: any; highlight: boolean }) {
  const plan = session.training_plans
  const client = plan?.clients
  const log = session.session_logs?.[0]
  const isCompleted = log?.status === 'completed'
  const isInProgress = log?.status === 'in_progress'
  const exerciseCount = session.plan_session_exercises?.length ?? 0
  const time = formatTime(session.session_time)
  const level = plan?.level

  return (
    <Link
      href={`/dashboard/hoy/${session.id}`}
      className={`block rounded-2xl border transition-colors overflow-hidden
        ${highlight ? 'bg-[#1C1C1C]' : 'bg-[#161616]'}
        ${isCompleted
      ...
      <div className="flex items-center gap-4 p-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            ...
            {level && (
              <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border shrink-0 ${LEVEL_COLORS[level]}`}>
                {LEVEL_LABELS[level]}
              </span>
            )}
          </div>
          <div className="flex items-center gap-3 text-xs text-white/40">
            {plan?.type === 'group' ? (
              <span className="flex items-center gap-1">
                ...
                Grupal · Sesión {session.session_label}
              </span>
            ) : (
              <span className="flex items-center gap-1">
